...
(function() {
    ...
    async function initElement() {
        ...
        if (typeof layer == 'undefined') {
            $('head').append('<script src="https://lf6-cdn-tos.bytecdntp.com/cdn/expire-1-M/layer/3.5.1/layer.js"></script>') // 名称：layer，版本：3.5.1，原始地址：https://www.layuicdn.com/#Layer
        }
        // 等待layer加载成功
        while (true) {
            if (typeof layer != 'undefined') {
        ...
        ctrl_btn.style.background = 'url(https://p1.meituan.net/csc/a891fae827e5ade9be6883eafb21b1cc3773.png)' //'url(https://vkceyugu.cdn.bspapp.com/VKCEYUGU-7c225101-813b-41f0-bcda-b99f9ef1c986/70ae58b2-52aa-47d3-b5d1-6a1d6d168d6b.png)';
                ...
                if (btn_txt=="搜索") {
                    ...
                    var search_iframe = $(`
                        <div style='margin-top:1rem;display: flex; flex-wrap: nowrap;' id='search_div_${btn_num}'>
                            <div class='layui-col-xs8 layui-col-sm8 layui-col-md8'><input type="text" name="url" value='${search_content}' placeholder="请输入搜索内容" autocomplete="off" class="layui-input" id='url_input_${btn_num}'></div>
                            <button type="button" class="layui-btn layui-btn-normal layui-col-xs2 layui-col-sm2 layui-col-md2" id='gotourl_btn_${btn_num}'>百度</button>
                            <button type="button" class="layui-btn layui-btn-normal layui-col-xs2 layui-col-sm2 layui-col-md2" id='gotourl_btn2_${btn_num}'>必应</button>
                        </div>
                    ...
                    $(`#gotourl_btn2_${btn_num}`).on('click', function () {
                        var url_input = $(`#url_input_${btn_num}`).val()
                        $(`#search_iframe_${btn_num}`).attr('src', `${search_url2}${encodeURI(url_input)}`)
                    })
